...
import { setAcName, setStatus, setTotalAmt, setVrDate, setVrNo } from "../redux/headerTableSlice";
import { acNameSchema, statusSchema, vrNoSchema } from "../validation/headerValidation";
...
const Header = () => {
  ...
  const handleAcName = async(e)=>{
    ...
    try{
      if(!value){
        dispatch(setAcName(''))
        setErrors((prevErrors) => ({ ...prevErrors, acName: 'Ac name required' }));
        return
      }
  ...
  const handleVrNo = async(e)=>{
    ...
    try{
      if(!value){
        dispatch(setVrNo(''))
        setErrors((prevErrors) => ({ ...prevErrors, vrNo:'Voucher No. required'}));
        return
      }
      ...
      <div className="container p-10 px-24">
        <div className="flex  justify-between">
          <div className="flex flex-col">
            ...
            {errors.vrNo && <div className="text-red-500 ml-16 mt-1">{errors.vrNo}</div>}
          </div>
          <div className="flex">
            <p>Vr Date :-</p>
            <input
          ...
          <div className="flex flex-col">
            ...
            {errors.status && <div className="text-red-500 ml-16 mt-1">{errors.status}</div>}
          </div>
        ...
        <div className="flex mt-10 justify-between">
          <div className="flex flex-col"> 
            ...
            {errors.acName && <div className="text-red-500 ml-20 mt-1">{errors.acName}</div>}
          </div>
          <div className="flex">
            <p>Ac Amt</p>
            <input
